...
def transform_to_onnx(cfgfile, weightfile, batch_size=1, onnx_file_name=None):
    weight_name = os.path.basename(weightfile)
    weight_name, _ = os.path.splitext(weight_name)
    model = Darknet(cfgfile, inference=True)
    ...
    if dynamic:
        ...
        if not onnx_file_name:
            ...
            onnx_file_name = "{}_-1_3_{}_{}_dynamic.onnx".format(weight_name, model.height, model.width)
        ...
        onnx_file_name = "{}_{}_3_{}_{}_static.onnx".format(weight_name, batch_size, model.height, model.width)
        torch.onnx.export(model,
                          x,
                          onnx_file_name,
                          export_params=True,
