...
model_onnx = Model_onnx(opt)
model_onnx.load_state_dict(torch.load(savefile_for_ONNX))
# model_onnx = torch.load(savefile_for_ONNX)

onnxfile = "OCR_test.onnx"

# torch.onnx.export(model, (dummy_input, text_for_pred), onnxfile, opset_version=12)
input_names = ["input_ocr"]
output_names = ["output_ocr"]
dynamic_axes = {"input_ocr" : {0 : "batch_size"}, "output_ocr" : {0 : "batch_size"}}
