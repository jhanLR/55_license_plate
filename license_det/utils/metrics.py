...
def ap_all_class(tp, conf, pred_cls, target_cls, plot=False, fname='precision-recall_curve.png'):
    ...
    # Find unique classes
    ttt = np.zeros(target_cls.shape[0])
    ppp = np.zeros(pred_cls.shape[0])
    unique_classes = np.unique(ttt)
