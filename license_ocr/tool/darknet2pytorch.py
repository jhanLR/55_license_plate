...
from tool.region_layer2 import RegionLayer2
from tool.yolo_parameter import YoloParameter
...
# for detect layer & remove postpocessing for make only backbone
...
class Darknet(nn.Module):
    def __init__(self, cfgfile, inference=False):
        ...
        # self.version = np.array([0,0,0], dtype=np.int32)  # (int32) version info: major, minor, revision
        # self.seen = np.array([0], dtype=np.int64)  # (int64) number of images seen during training

    def forward(self, x):
        self.training = not self.inference
        head_layer_checker = "None"
        ind = -2
        self.loss = None
        outputs = dict()
        out_boxes = []
        ...
        # region type 사용 시
        if head_layer_checker == 'region':
            print("region layer is head")
            return get_region_boxes(out_boxes)

        # yolo type 사용 시
        elif head_layer_checker == 'yolo':
            print("yolo layer is head")
            print(self.training)
            if self.training:
                return out_boxes
            else:
        ...
        for block in blocks:
            if block['type'] == 'net':
                ...
                if activation == 'leaky':
                    ...
                    model.add_module('swish{0}'.format(conv_id), Swish())
                elif activation == 'silu':
                    model.add_module('silu{0}'.format(conv_id), nn.SiLU())
                ...
                # if darknet -> activation == linear
                # if pytorch -> activation == logistic
                ...
                layers = block['layers'].split(',')
                ind = len(models)
                # print("INFO_bf ",layers, ind)
                layers = [int(i) if int(i) > 0 else int(i) + ind for i in layers]
                # print("INFO_at" ,layers, ind)
                if len(layers) == 1:
                    if 'groups' not in block.keys() or int(block['groups']) == 1:
                    ...
                    # print(layers[0], ind - 1)
                    assert (layers[0] == ind - 1 or layers[1] == ind - 1)
                    prev_filters = out_filters[layers[0]] + out_filters[layers[1]]
                    prev_stride = out_strides[layers[0]]
                elif len(layers) == 3:
                    # print(layers[0], ind - 1)
                    assert (layers[0] == ind - 1 or layers[1] == ind - 1 or layers[2] == ind - 1)
                    prev_filters = out_filters[layers[0]] + out_filters[layers[1]] + out_filters[layers[2]]
                    prev_stride = out_strides[layers[0]]
                elif len(layers) == 4:
                    # print(layers[0], ind - 1)
                    assert (layers[0] == ind - 1 or layers[1] == ind - 1 or layers[2] == ind - 1 or layers[3] == ind - 1)
                    prev_filters = out_filters[layers[0]] + out_filters[layers[1]] + out_filters[layers[2]] + \
                    ...
                    assert (layers[0] == ind - 1 or layers[1] == ind - 1 or layers[2] == ind - 1 or layers[3] == ind - 1 
                            or layers[4] == ind -1)
                    prev_filters = out_filters[layers[0]] + out_filters[layers[1]] + out_filters[layers[2]] + \
                                   out_filters[layers[3]] + out_filters[layers[4]]
                    prev_stride = out_strides[layers[0]]
                elif len(layers) == 6:
                    # print(layers[0], ind - 1)
                    assert (layers[0] == ind - 1 or layers[1] == ind - 1 or layers[2] == ind - 1 or layers[3] == ind - 1
                            or layers[4] == ind -1 or layers[5] == ind - 1)
                    prev_filters = out_filters[layers[0]] + out_filters[layers[1]] + out_filters[layers[2]] + \
                                   out_filters[layers[3]] + out_filters[layers[4]] + out_filters[layers[5]]
                    prev_stride = out_strides[layers[0]]     
                else:
                    print("route error!!!")
                # print("____________________")
                ...
                # out_strides.append(prev_stride)
                # models.append(loss)
                region_layer = RegionLayer2()
                anchors = block['anchors'].split(',')
                # anchor_mask = ['0', '1', '2', '3', '4']
                # region_layer.anchor_mask = [int(i) for i in anchor_mask]
                ...
                region_layer.stride = prev_stride
                region_layer.scale_x_y = 1.0
                ...
                # print('yolo',self.training)
                yolo_layer.training = True
    ...
    def load_weights(self, weightfile):
        ...
        # self.version = np.fromfile(fp, dtype=np.int32, count=3)  # (int32) version info: major, minor, revision
        # self.seen = np.fromfile(fp, dtype=np.int64, count=1)  # (int64) number of images seen during training
