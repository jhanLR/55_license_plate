...
def plot_one_box2(x, img, color=None, label=None, line_thickness=None):
    ...
    cv2.rectangle(img, (int(x[0]), int(x[1])-(int(x[3])-int(x[1]))), (int(x[2]), int(x[3])-(int(x[3])-int(x[1]))), color, -1, cv2.LINE_AA)  # filled
    
    img = Image.fromarray(img)
    draw = ImageDraw.Draw(img)
    font=ImageFont.truetype("MonoplexKR-Text.ttf",20)
    org=(50,100)
    text=str(label)
    color2 = (0,0,0)
    draw.text((int(x[0]), int(x[1])-21),text,font=font,fill=color2) #text를 출력
