    ...
    for b in range(nB):
        for t in range(50):
            ...
            for n in range(nA):
                ...
                iou = bbox_ious(anchor_box, gt_box, x1y1x2y2=False)
                if anchor_step == 4:
            ...
            iou = bbox_ious(gt_box, pred_box, x1y1x2y2=False)  # best_iou
...
class RegionLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        return loss
        # boxes = torch.cat((pred_boxes[0], pred_boxes[2], pred_boxes[3], pred_boxes[4]), dim=2)
        # return boxes