...
def yolo_forward_pytorch(output, conf_thresh, num_classes, anchors, num_anchors, scale_x_y, only_objectness=1,
                              validation=False):
...
def yolo_forward_LR(output, conf_thresh, num_classes, anchors, num_anchors, scale_x_y, only_objectness=1,
                              validation=False):
    # Output would be invalid if it does not satisfy this assert
    # assert (output.size(1) == (5 + num_classes) * num_anchors)
    ...
    # Tiny Model (yolov7 tiny) -> Fin
    bxy = bxy * 2 - 0.5
    bwh = torch.pow(bwh * 2, 2)
    
    # Heavy Model (yolov7) -> Fin
    # bxy = bxy * scale_x_y - 0.5 * (scale_x_y - 1)
    # bwh = torch.pow(bwh * 2, 2)
    
    # Test Pytorch Model -> Yet Not
    # bxy = torch.sigmoid(bxy) * 2 - 0.5
    # # bwh = (torch.sigmoid(bwh) * 2) ** 2
    # bwh = torch.pow(torch.sigmoid(bwh) * 2, 2)
    # det_confs = torch.sigmoid(det_confs)
    # cls_confs = torch.sigmoid(cls_confs)
...
class YoloLayer(nn.Module):
    ...
    def forward(self, output, target=None):
        ...
        # return yolo_forward_pytorch(output, self.thresh, self.num_classes, masked_anchors, len(self.anchor_mask),scale_x_y=self.scale_x_y)
